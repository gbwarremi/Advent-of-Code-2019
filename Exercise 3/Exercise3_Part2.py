...
def calculate_grid(coordinates, x, y, input_number, intersections = []):
    ...
    for i in coordinates:
        ...
        distance = int(i[1:])
        if direction == 'R':
            for point in range(x, x+distance, 1):
                x += 1
                steps += 1
                grid[y][x] = 1
                if len(intersections) > 0:
                    intersections = check_intersection(intersections, y, x, input_number, steps)
        elif direction == 'L':
            for point in range(x, x-distance, -1):
                x -= 1
                steps += 1
            ...
            for point in range(y, y+distance, 1):
                y += 1
                steps += 1
                grid[y][x] = 1
                if len(intersections) > 0:
...
def check_intersection(intersections, x, y, input_number, steps):
    for i in range(len(intersections)):
        if x == intersections[i][0] and y == intersections[i][1] and intersections[i][input_number+1] == 0:
            intersections[i][input_number+1] = steps
    return intersections


grids.append(calculate_grid(input_data,x ,y,1))
grids.append(calculate_grid(input_data_2,x ,y,2))
...
for grid in range(len(grids)-1):
    ...
    for row in range(rows-1, 0, -1):
        for column in range(columns):
            if (grids[grid][row][column] == 1 and grids[grid+1][row][column] == 1):
                manhattan_distance = abs(row - x) + abs(column - y)
                intersections.append([row, column, 0, 0])
                print(f'Intersection found at {row}, {column}, manhattan distance of: {manhattan_distance}')
                if manhattan_distance < closest_intersection or closest_intersection == 0:
                    closest_intersection = manhattan_distance
print(f'The closest intersection was at: {closest_intersection}')

intersections = calculate_grid(input_data,x ,y,1, intersections)
intersections = calculate_grid(input_data_2,x ,y,2, intersections)
closest_steps = 0

for i in intersections:
    steps = i[2] + i[3]
    print(f'Intersections at {i[0]}, {i[1]} has total steps of {steps}')
    if steps < closest_steps or closest_steps == 0:
        closest_steps = steps
print(f'The closest steps were: {closest_steps}')
